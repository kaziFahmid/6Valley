...
{servicesList?.map((item,index)=><div key={index} className='text-center px-[26px]' style={{borderRight:item?.img2?"none": '0.5px solid #0000004D'}}>
    <div className='flex justify-center  items-center gap-[11px]'> 
        ...
        {item?.img2&&<Image src={item?.img2} width={30} height={30} alt={item.title} />}
    </div>
    <h3 className='mt-[12.5px] font-bold text-[16px] text-[var(--text-color)]'>{item.title}</h3>
    <p className='mt-[15px] font-normal text-[12px] text-[var(--service-des-color)]'>{item.des}</p>
</div>)}
</div>
